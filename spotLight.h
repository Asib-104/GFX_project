...
public:
    ...
    SpotLight(float posX, float posY, float posZ, float dirX, float dirY, float dirZ,
        float ambR, float ambG, float ambB, float diffR, float diffG, float diffB,
        float specR, float specG, float specB, float constant, float linear, float quadratic,
        float cutOffAngle, float outerCutOffAngle, int num) {
    ...
    void setUpSpotLight(Shader& lightingShader)
    {
        ...
        if (lightNumber == 1) {
            ...
            lightingShader.setFloat("spotLights[0].cutOff", cutOff);
            lightingShader.setFloat("spotLights[0].outerCutOff", outerCutOff);
        }
        else if (lightNumber == 2) {
            lightingShader.setVec3("spotLights[1].position", position);
            lightingShader.setVec3("spotLights[1].direction", direction);
            ...
            lightingShader.setFloat("spotLights[1].cutOff", cutOff);
            lightingShader.setFloat("spotLights[1].outerCutOff", outerCutOff);
        }
    }
